...
const StartQuiz = () => {
  ...
  useEffect(() => {
    axiosInstance
      .get(`http://localhost:5000/questions?quizId=${id}`)

      .then((response) => {
        console.log(response.data);
        setQuizData(response.data);

        return axiosInstance.get(`http://localhost:5000/quizzes/${id}`);
      })
      .then((response) => {
  ...
  const handleOptionChange = (value, checked) => {
    ...
    if (currentQuestion.type === "single") {
      ...
      const currentAnswers = new Set(updatedAnswers[currentQuestion.id] || []);
      if (checked) {
        currentAnswers.add(value);
      } else {
        currentAnswers.delete(value);
      }
      updatedAnswers[currentQuestion.id] = [...currentAnswers];
    }
  ...
  const calculateScore = () => {
    ...
    quizData.forEach((question) => {
      const userAnswers = answers[question.id] || []; // Get the user's answers for the current question
                  ...
                  <FormControlLabel
                    control={
                      currentQuestion.type === "single" ? (
                        <Radio
                          checked={
                            answers[currentQuestion.id]?.[0] === option.text ||
                            false
                          }
